...
if (select == "2")//rhyta.com ve ya dayrep.com
{
    var result = debtors.Where(debtors => debtors.Email.Contains("rhyta.com") || debtors.Email.Contains("dayrep.com")).ToList();
    result.ForEach(result => Console.WriteLine(result));
}
else if (select == "3")//Yashi 26-dan 36-ya qeder olan borclulari cixartmag
{
    foreach (var item in debtors)
        if (DateTime.Now.Year - item.BirthDay.Year is >= 26 and <= 36)
        {
            if (DateTime.Now.Month < item.BirthDay.Month)
                Console.WriteLine(item.ToString());
            else if (DateTime.Now.Month == item.BirthDay.Month)
                if (DateTime.Now.Day < item.BirthDay.Day)
                    Console.WriteLine(item.ToString());
        }
}
else if (select == "4")//Borcu 5000-den cox olmayan borclularic cixartmag
{
    foreach (var item in debtors)
        if (item.Debt <= 5000)
            Console.WriteLine(item.ToString());
}
else if (select == "5")//Butov adi 18 simvoldan cox olan ve telefon nomresinde
                       //2 ve ya 2-den cox 7 reqemi olan borclulari cixartmaq
{
    foreach (var item in debtors)
    {
        ...
        {
            int sayi = 0;
            for (global::System.Int32 i = 0; i < item.Phone.Length; i++)
                if (item.Phone[i] == '7')
                    sayi++;
            if (sayi >= 2)
                Console.WriteLine(item.ToString());
...
else if (select == "6")//Qishda anadan olan borclulari cixardmaq 
{
    debtors.ForEach(d =>
    {
        if (d.BirthDay.Month is 12 or 1 or 2)
            Console.WriteLine(d.ToString());
    }
    );
}

else if (select == "7")//Borcu, umumi borclarin orta borcunnan cox olan borclulari cixarmaq ve
                       //evvel familyaya gore sonra ise meblegin azalmagina gore sortirovka etmek
{

    int total = 0;
    debtors.ForEach(d => { total += d.Debt; });
    int result = total / debtors.Count;

    List<Debtor> dbs = new List<Debtor>();
    List<string> dbsFullnames = new List<string>();
    List<int> debtAmount = new List<int>();
    foreach (var item in debtors)
        if (item.Debt > result)
        {
            dbs.Add(item);
            dbsFullnames.Add(item.FullName);
            debtAmount.Add(item.Debt);
        }
    debtAmount.Sort();
    dbsFullnames.Sort();
    ...
    do
    {
        ...
        {
            if (dbsFullnames[count].ToString() == item.FullName)
            {

                Console.WriteLine(item.ToString());

                count++;
                if (count == debtAmount.Count)
                {
                    stop = false; 
                    break;
                }
...
else if (select == "8")//Telefon nomresinde 8 olmayan borclularin yalniz familyasin,yashin ve umumi borcun meblegin cixarmaq
{
    foreach (var item in debtors)
    {
        if (!item.Phone.Contains('8'))
        {
            ...
            if (DateTime.Now.Month < item.BirthDay.Month)
                Console.Write(" " + (DateTime.Now.Year - item.BirthDay.Year));
            else if (DateTime.Now.Month == item.BirthDay.Month)
            {
                if (DateTime.Now.Day < item.BirthDay.Day)
                    Console.Write(" " + (DateTime.Now.Year - item.BirthDay.Year));
                ...
                Console.Write(" " + (DateTime.Now.Year - item.BirthDay.Year - 1));

            Console.WriteLine(" " + item.Debt);
        }
    }
}
else if (select == "9")//Adinda ve familyasinda hec olmasa 3 eyni herf olan borclularin
                       //siyahisin cixarmaq ve onlari elifba sirasina gore sortirovka elemek
{
    ...
    foreach (var item in debtors)
    {
        for (global::System.Int32 i = 0; i < alpha.Length; i++)
        {
            int count = 0;
            for (global::System.Int32 j = 0; j < item.FullName.Length; j++)
                if (alpha[i] == item.FullName.ToUpper()[j])
                    count++;
            if (count >= 3)
            {
                dbs.Add(item);
    ...
    do
    {
        foreach (var item in dbs)
        {
            if (fullnames[index] == item.FullName)
            {
                Console.WriteLine(item.ToString());
                index++;
                if (index == dbs.Count)
...
else if (select == "10")//borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq
{
    List<int> years = new List<int>();
    foreach (var item in debtors)
    {
        years.Add(item.BirthDay.Year);
    }
    int most = 0;
    int mostYear = 0;
    for (global::System.Int32 i = 0; i < years.Count; i++)
    {
        int plus = 0;
        foreach (var item in debtors)
        {
            if (years[i] == item.BirthDay.Year)
            {
                plus++;
                if (plus > most)
                {
...
else if (select == "11")//Borcu en boyuk olan 5 borclunun siyahisini cixartmaq
{
    List<int> debtAmount = new List<int>();
    foreach (var item in debtors)
    {
    ...
    debtAmount.Sort();
    int[] debs_5 = new int[5];
    int x = 0;
    for (global::System.Int32 i = debtAmount.Count - 1; i > debtAmount.Count - 6; i--)
    {
        debs_5[x++] = debtAmount[i];
    }
    int index = 0;
    do
    {
        foreach (var item in debtors)
        {
            if (debs_5[index] == item.Debt)
            {
                Console.WriteLine(item.ToString());
                index++;
                if (index == debs_5.Length)
                    return;
            }
...
else if (select == "12")//Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq
{
    int total = 0;
    foreach (var item in debtors)
    {
...
else if (select == "13")//2ci dunya muharibesin gormush borclularin siyahisi cixartmaq
{
    var dbs = debtors.Where((dbs) => dbs.BirthDay.Year < 1945).ToList();
    dbs.ForEach(dbs => Console.WriteLine(dbs.ToString()));
}
else if (select == "14")//Nomresinde tekrar reqemler olmayan borclularin ve
                        //onlarin borcunun meblegin cixartmaq
{
    bool other = false;
    foreach (var item in debtors)
    {
        ...
        for (global::System.Int32 i = 0; i < item.Phone.Length; i++)
        {
            for (global::System.Int32 j = 0; j < item.Phone.Length; j++)
            {
                if (item.Phone[i] == item.Phone[j])
                {
                    count++;
                }
            }
...
else if (select == "15")//Tesevvur edek ki,butun borclari olanlar bugunden etibaren her ay 500 azn pul odeyecekler.
                        //Oz ad gunune kimi borcun oduyub qurtara bilenlerin siyahisin cixartmaq
{
    var result = debtors.Where(
        (Debtor d) =>
    {
    ...
    result.ForEach(d => Console.WriteLine(d.ToString()));
}
else if (select == "16")//Adindaki ve familyasindaki herflerden
                        //"smile" sozunu yaza bileceyimiz borclularin siyahisini cixartmaq
{
    var result = debtors.Where((Debtor d) =>
    {
        string s = "smile";
        for (global::System.Int32 i = 0; i < s.Length; i++)
            if (!d.FullName.ToLower().Contains(s[i]))
                return false;
...
class Debtor
{
    ...
    public override string ToString() => $"{this.FullName} {this.BirthDay.ToShortDateString()} {this.Phone} {this.Email} {this.Address} {this.Debt}";
